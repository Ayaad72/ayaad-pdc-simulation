...
export default function ParallelSimulation() {
  const [mode, setMode] = useState<'sequential' | 'parallel' | 'distributed' | null>(null)
  ...
  const saveResult = (newMetrics: any, simulationMode: 'sequential' | 'parallel' | 'distributed') => {
  ...
  const startSequential = async () => {
    setMode('sequential')
    setIsRunning(true)
    setMetrics({ totalTime: 0, processingTime: 0, communicationDelay: 0, waitingTime: 0 })
          ...
          {activeTab === 'simulation' ? (
              ...
              {!mode && (
                <div className="mb-12">
                  <div className="text-center mb-12">
                    <h2 className="text-2xl md:text-3xl font-bold text-white mb-3">Choose a Processing Mode</h2>
                    <p className="text-slate-400 max-w-2xl mx-auto">
                      Explore how different computing paradigms handle the same task. Watch real-time animations showing sequential, parallel, and distributed execution.
                    </p>
                  </div>
              ...
              {mode && (
                <>
                  <div className="bg-slate-800 rounded-xl p-6 md:p-8 mb-6 border border-slate-700">
                    {mode === 'sequential' && <SequentialVisualization state={animationState} />}
                    {mode === 'parallel' && <ParallelVisualization state={animationState} />}
                    {mode === 'distributed' && <DistributedVisualization state={animationState} />}
                  </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-6 flex-col sm:flex-row gap-4">
                <div>
                  <h2 className="text-2xl md:text-3xl font-bold text-white">Results History</h2>
                  <p className="text-slate-400 mt-1">{results.length} simulation(s) recorded</p>
                </div>
                {results.length > 0 && (
                  <button
              ...
              {results.length === 0 ? (
                <div className="bg-slate-800 rounded-xl p-8 md:p-12 border border-slate-700 text-center">
                  <p className="text-slate-400 text-lg">No simulations recorded yet.</p>
                  <p className="text-slate-500 mt-2">Run a simulation to see results appear here.</p>
                </div>
              ) : (
                  ...
                  <table className="w-full text-sm md:text-base">
                    <thead>
                      <tr className="border-b border-slate-700 bg-slate-900">
                        <th className="px-4 md:px-6 py-3 text-left font-semibold text-white">Mode</th>
                        <th className="px-4 md:px-6 py-3 text-left font-semibold text-white">Total Time</th>
                        <th className="px-4 md:px-6 py-3 text-left font-semibold text-white hidden sm:table-cell">Processing Time</th>
                        <th className="px-4 md:px-6 py-3 text-left font-semibold text-white hidden md:table-cell">Waiting/Comm Time</th>
                        <th className="px-4 md:px-6 py-3 text-left font-semibold text-white">Date & Time</th>
                      </tr>
                    </thead>
                    <tbody>
                      {results.map((result) => (
                        <tr key={result.id} className="border-b border-slate-700 hover:bg-slate-750 transition">
                          <td className="px-4 md:px-6 py-4">
                            <span className={`px-3 py-1 rounded-full text-white text-xs md:text-sm font-medium ${
                          ...
                          <td className="px-4 md:px-6 py-4 font-semibold text-green-400">{result.totalTime}ms</td>
                          <td className="px-4 md:px-6 py-4 text-slate-300 hidden sm:table-cell">{result.processingTime}ms</td>
                          <td className="px-4 md:px-6 py-4 text-slate-300 hidden md:table-cell">
                            {result.waitingTime > 0 ? `${result.waitingTime}ms` : `${result.communicationDelay}ms`}
                          </td>
                          <td className="px-4 md:px-6 py-4 text-slate-400 text-xs md:text-sm">{result.timestamp}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {results.length > 0 && (
                <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
                  <ComparisonCard
                    label="Fastest Mode"
                    mode={results.reduce((a, b) => (a.totalTime < b.totalTime ? a : b)).mode}
                    time={Math.min(...results.map(r => r.totalTime))}
                  />
                  <ComparisonCard
                    label="Average Execution"
                    mode="average"
                    time={Math.round(results.reduce((sum, r) => sum + r.totalTime, 0) / results.length)}
                  />
                  <ComparisonCard
                    label="Total Runs"
                    mode="count"
  ...
  return (
    <div className="flex flex-col items-center justify-center py-8 md:py-12">
      <div className="text-center mb-8">
        <p className="text-slate-300 mb-4 text-sm md:text-base">Executing sequentially: one number at a time</p>
        {state.step && (
      ...
      {state.final && (
        <div className="mt-8 md:mt-12 text-center">
          <p className="text-slate-300 text-base md:text-lg mb-2">Sum of 1 to 12:</p>
          <div className="text-4xl md:text-5xl font-bold text-green-400">{state.currentSum}</div>
          <p className="text-slate-400 mt-2 text-sm md:text-base">Calculation complete!</p>
        </div>
      )}
    </div>
  )
...
function ParallelVisualization({ state }: { state: any }) {
  return (
    <div className="flex flex-col items-center justify-center py-8 md:py-12">
      <div className="text-center mb-8">
        <p className="text-slate-300 mb-4 text-sm md:text-base">Two processors work in parallel on shared memory</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-8 md:gap-16 mb-8 md:mb-12 w-full justify-center px-4">
        ...
        <div className="flex flex-col items-center">
          <div className="text-slate-300 mb-4 text-xs md:text-sm">Processor 1</div>
          <div
          ...
          <div className="mt-4 text-center">
            ...
            <p className="text-purple-400 font-bold text-lg">{state.processor1Number || '-'}</p>
            <p className="text-slate-400 text-xs md:text-sm mt-2">Sum: {state.sum1 || 0}</p>
          </div>
        </div>

        {/* Processor 2 */}
        <div className="flex flex-col items-center">
          <div className="text-slate-300 mb-4 text-xs md:text-sm">Processor 2</div>
          <div
            className={`relative w-24 h-24 md:w-28 md:h-28 rounded-lg border-4 flex items-center justify-center font-bold text-lg md:text-xl transition ${
          ...
          <div className="mt-4 text-center">
            <p className="text-slate-400 text-xs md:text-sm">Processing:</p>
            <p className="text-purple-400 font-bold text-lg">{state.processor2Number || '-'}</p>
            <p className="text-slate-400 text-xs md:text-sm mt-2">Sum: {state.sum2 || 0}</p>
          </div>
        </div>
      </div>
      ...
      {state.merging && (
        <div className="mb-8 text-center">
          ...
          <p className="text-slate-300 font-semibold text-sm md:text-base">Synchronizing & Merging Results</p>
        </div>
      )}
      ...
      {state.final && (
        <div className="mt-8 md:mt-12 text-center">
          <p className="text-slate-300 text-base md:text-lg mb-2">Final Sum (P1 + P2):</p>
          <div className="text-4xl md:text-5xl font-bold text-green-400">{state.currentSum}</div>
          <p className="text-slate-400 mt-2 text-sm md:text-base">Parallel execution complete!</p>
        </div>
      )}
    </div>
  )
...
function DistributedVisualization({ state }: { state: any }) {
  return (
    <div className="flex flex-col items-center justify-center py-8 md:py-12">
      <div className="text-center mb-8">
        <p className="text-slate-300 mb-4 text-sm md:text-base">Two distributed systems communicate via message passing</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-8 md:gap-16 lg:gap-32 mb-8 md:mb-12 relative w-full justify-center px-4">
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="mt-4 text-center">
            <p className="text-slate-400 text-xs md:text-sm">Processing:</p>
            <p className="text-orange-400 font-bold text-lg">{state.processor1Number || '-'}</p>
            <p className="text-slate-400 text-xs md:text-sm mt-2">Sum: {state.sum1 || 0}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="mt-4 text-center">
            <p className="text-slate-400 text-xs md:text-sm">Processing:</p>
            <p className="text-orange-400 font-bold text-lg">{state.processor2Number || '-'}</p>
            <p className="text-slate-400 text-xs md:text-sm mt-2">Sum: {state.sum2 || 0}</p>
          </div>
        </div>
      ...
      {state.communicating && (
        <div className="mb-8 text-center">
          ...
          <p className="text-slate-300 font-semibold text-sm md:text-base">Transmitting data via network...</p>
          <p className="text-slate-400 text-xs md:text-sm mt-2">(Simulating network delay)</p>
        </div>
      )}
      ...
      {state.final && (
        <div className="mt-8 md:mt-12 text-center">
          <p className="text-slate-300 text-base md:text-lg mb-2">Final Sum (Received):</p>
          <div className="text-4xl md:text-5xl font-bold text-green-400">{state.currentSum}</div>
          <p className="text-slate-400 mt-2 text-sm md:text-base">Message passing complete!</p>
        </div>
      )}
    </div>
  )
        ...
        <p className="text-slate-300 text-sm md:text-base">
          {mode === 'sequential' && (
            <>
              ...
              No parallelism, pure sequential execution. All processing happens on a single CPU unit.
            </>
          )}
          {mode === 'parallel' && (
            <>
              <strong className="text-purple-400">Parallel Processing:</strong> Two processors share memory and work on different data
              simultaneously. Both finish their tasks and synchronize before merging results. Watch how
              total time is roughly half of sequential!
            </>
          )}
          {mode === 'distributed' && (
            <>
              <strong className="text-orange-400">Distributed Processing:</strong> Two independent systems process data in parallel,
              then communicate results via message passing. The network delay adds to total execution time.
              This simulates real-world distributed systems like microservices.
            </>
          )}
        </p>
      </div>
